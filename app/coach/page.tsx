  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
        ...
        {onInfo && (
          <button onClick={onInfo} className="tc-press" aria-label={`${label} 설명`} style={{ WebkitAppearance: 'none', appearance: 'none', WebkitTapHighlightColor: 'transparent', boxSizing: 'border-box', width: 12, height: 12, flex: '0 0 auto', borderRadius: '50%', border: `1px solid ${infoActive ? 'rgba(250,178,25,.7)' : 'rgba(250,178,25,.45)'}`, background: infoActive ? 'rgba(250,178,25,.2)' : 'transparent', color: '#fab219', fontSize: 8, fontWeight: 800, lineHeight: 1, cursor: 'pointer', display: 'grid', placeItems: 'center', padding: 0 }}>{infoActive ? '✕' : '?'}</button>
        )}
      </div>
