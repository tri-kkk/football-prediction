'use client';
// app/coach/settings/page.tsx — 설정: 멤버쉽 상태 · 뱅크롤 · 계정.
import { useEffect, useState } from 'react';
import { coachApi, MembershipError } from '@/lib/coachApi';
...
export default function SettingsPage() {
  ...
  const logout = () => { localStorage.removeItem('coach_token'); location.href = '/coach'; };
  ...
  return (
    <>
      ...
      ) : row('상태', member === null ? '확인 중…' : '멤버쉽 이용 중 ✓')}
      ...
      {sect('계정')}
      {row('로그인', 'Google · TrendSoccer 통합')}
      <button onClick={logout} style={{ width: '100%', border: '1px solid rgba(255,255,255,.1)', background: '#1a1a19', color: '#e66767', fontWeight: 700, fontSize: 13, padding: 13, borderRadius: 13, cursor: 'pointer' }}>로그아웃</button>
