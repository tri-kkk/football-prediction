...
export default function UnifiedFeed({ matches, filter, onFilterChange, loading, leagueFilter, onClearLeagueFilter, dateFilter = 'all', onDateFilterChange, insertAfter, insertSlot }: Props) {
  ...
  return (
    <section className="space-y-3">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2 flex-wrap">
            ...
            {leagueInfo && (
              <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-sm font-medium">
                ...
                {leagueInfo.name}
                {onClearLeagueFilter && (<button type="button" onClick={onClearLeagueFilter} className="ml-1 text-emerald-300/80 hover:text-white text-base leading-none" aria-label="리그 필터 해제">×</button>)}
              </span>
            )}
          </h2>
          <p className="text-xs text-gray-500 mt-0.5">
            {curated.live.length > 0 && (<span className="text-red-400 font-bold mr-2">🔴 LIVE {curated.live.length}경기</span>)}
            <span>전체 {totalShowable}경기 · 상위 {displayList.length}개 표시</span>
          </p>
        </div>
        {leagueInfo && (
      ...
      {loading && (<div className="text-center py-8 text-gray-500 text-sm">경기 정보를 불러오는 중…</div>)}
      {!loading && displayList.length === 0 && (
        <div className="text-center py-10 rounded-xl bg-gray-900/40 border border-gray-800">
          <p className="text-gray-400 text-sm">표시할 경기가 없습니다.</p>
          <p className="text-xs text-gray-600 mt-1">다른 종목 탭을 선택해보세요.</p>
        </div>
      )}
      {!loading && displayList.length > 0 && (() => {
      ...
      {!loading && totalShowable > 0 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          {hasMore && !expanded && (<button onClick={() => setExpanded(true)} className="px-4 py-2 rounded-lg bg-gray-900/60 hover:bg-gray-800 border border-gray-800 text-sm text-gray-300 hover:text-white transition-colors">더보기 ({totalShowable - displayList.length}경기)</button>)}
        </div>
      )}
    </section>
  )
