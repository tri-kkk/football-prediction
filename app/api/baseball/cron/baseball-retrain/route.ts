...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. 정규시즌 누적 경기 수 확인 (최소 20개 이상일 때 재학습)
    ...
    if (!totalCount || totalCount < 20) {
      return NextResponse.json({
        success: true,
        message: `정규시즌 데이터 부족 (${totalCount}개) - 재학습 스킵. 20개 이상 필요`,
        totalCount,
      })
    }
