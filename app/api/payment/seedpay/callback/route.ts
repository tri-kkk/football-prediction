  ...
  try {
    ...
    if (!merchantKey || !mid) {
      ...
      return NextResponse.redirect(
        `${baseUrl}/premium/pricing/result?status=error&message=${encodeURIComponent('설정 오류')}`,
        { status: 303 }
      )
    }
    ...
    if (data.signData !== calculatedSignData) {
      ...
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.trendsoccer.com'
      return NextResponse.redirect(
        `${baseUrl}/premium/pricing/result?status=failed&message=${encodeURIComponent('결제 검증 실패')}`,
        { status: 303 }
      )
    }
...
// ✅ POST 요청 처리 (SeedPay가 POST로 보낼 때)
